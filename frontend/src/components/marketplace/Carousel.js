...
const CarouselButton = ({ useTriangle, color, disabled, clickHandler, icon, isLeftIcon }) => (
    <div className={`carousel-${isLeftIcon ? 'left' : 'right'}-arrow carousel-control`}>
        <div
            className={useTriangle ? `${isLeftIcon ? 'left' : 'right'}` : ''}
            style={{
                ...(isLeftIcon
        ...
        />
        <button
            className={!useTriangle ? `padding-${isLeftIcon ? 'left' : 'right'}-15` : ' mx-3 color-white'}
            disabled={disabled}
...
const CarouselIndicators = ({
    ...
    <ol className={`carousel-indicators `}>
        {slides.map((el, i) => (
            <li
                key={i}
                className={i === nextActiveIndex ? 'active mx-1' : 'mx-1'}
...
const Carousel = ({
    ...
    return (
        <div
            className="carousel-slider-wrapper mx-auto my-3"
            style={{
                ...
                icon={
                    leftIcon ? (
                        ...
                        <svg
                            ...
                            viewBox="0 0 256 512"
                            fill={useRightLeftTriangles ? '#ffffff' : '#000000'}
                        >
